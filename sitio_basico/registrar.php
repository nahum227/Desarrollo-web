...
registrar_user($nueva_id, $username, $password, $nombre, $apellido, $deportista);  //esta funcion registra al usuario en el archivo
...
//FUNCIONES
function registrar_user($id , $username , $password, $nombre, $apellido, $deportista){
    $file=fopen("archivos/users.txt","a");  //se abre el arhcivo users.txt con la opcion para escritura que no sobreescriba nada 

    $datos=$id.":".$username.":".$password.":".$nombre.":".$apellido.":".$deportista; //se concatenan todos los datos(el nuevo registro)
